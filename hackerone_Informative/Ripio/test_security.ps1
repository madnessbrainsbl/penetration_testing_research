$token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..Nf8x8jNII4oEVlhFAVkNz8mBNGlCOgmDUrT1X0TVF83Wr8xHVB0kFVT58aUz_A9KXKK_G0Ca04rAFWflzFWMVM5CaUsrxwq0dkpDlx-yvfUvN1AX7PzmgEqOM4o9JnT6DOmdfMnJFyT6vhJ5Z3sSMAyahiX_tXQJhnowdDX1HUlz_EzQzvzE_O9rW4pj6LWrGcHQbohUQm_79pUuUYkUSJQf7TyulHFawbn2BS69K3q-iSVgeAWgo5NFwjwQ5Wy9NSPwkZCNFDcnEeMw27vToNO0CgEu58YMXljUVz3-DS8-i6HHME0qWcbAEQIKAy2g5hzeRUgUY6L4lf-cwi_tWQ"

$headers = @{
    Authorization = "Bearer $token"
    "Content-Type" = "application/json"
}

# Test security settings endpoint
Write-Host "=== Testing POST /api/users/security/ ===" -ForegroundColor Cyan
try {
    $body = '{"security_enabled": false}'
    $response = Invoke-RestMethod -Uri "https://auth.ripio.com/api/users/security/" -Headers $headers -Method POST -Body $body -ErrorAction Stop
    Write-Host "SUCCESS:" -ForegroundColor Green
    $response | ConvertTo-Json -Depth 3
} catch {
    Write-Host "Error: $($_.Exception.Response.StatusCode) - $($_.Exception.Message)" -ForegroundColor Red
    $reader = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())
    $reader.ReadToEnd()
}

# Test change password
Write-Host "`n=== Testing POST /api/users/change-password/ ===" -ForegroundColor Cyan
try {
    $body = '{"old_password": "test", "new_password": "newtest123"}'
    $response = Invoke-RestMethod -Uri "https://auth.ripio.com/api/users/password/" -Headers $headers -Method POST -Body $body -ErrorAction Stop
    Write-Host "SUCCESS:" -ForegroundColor Green
    $response
} catch {
    Write-Host "Error: $($_.Exception.Response.StatusCode) - $($_.Exception.Message)" -ForegroundColor Red
    $reader = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())
    $reader.ReadToEnd()
}

# Test OTP disable
Write-Host "`n=== Testing POST /api/users/otp/disable/ ===" -ForegroundColor Cyan
try {
    $body = '{}'
    $response = Invoke-RestMethod -Uri "https://auth.ripio.com/api/v2/otp/disable/" -Headers $headers -Method POST -Body $body -ErrorAction Stop
    Write-Host "SUCCESS:" -ForegroundColor Green
    $response
} catch {
    Write-Host "Error: $($_.Exception.Response.StatusCode) - $($_.Exception.Message)" -ForegroundColor Red
}
