...
print("Getting server time...", flush=True)
...
for ep, base_params in endpoints_to_test:
    print(f"\nChecking {ep}...", flush=True)
    
    # Baseline
    base_res = send_signed_request(ep, base_params)
    if base_res.get('retCode') != 0:
        print(f"  Skipping (Baseline failed: {base_res.get('retCode')})", flush=True)
        continue
    ...
    if attack_dump == base_dump:
        print("  ✅ Safe (Param ignored)", flush=True)
    else:
        print("  🚨 DIFFERENCE FOUND!", flush=True)
        print(f"  Base: {len(base_dump)} chars", flush=True)
        print(f"  Attack: {len(attack_dump)} chars", flush=True)

# 2. HTTP Method Override (Auth Bypass) on Mainnet
print("\n[2] Testing HTTP Method Override on Mainnet", flush=True)
cmd_override = [
    "curl", "-s", "-X", "OPTIONS",
...
print(f"Status: (check stdout length)", flush=True)
if len(res_ov.stdout) == 0:
    print("  ✅ Safe (Empty response / CORS only)", flush=True)
elif "retCode" in res_ov.stdout:
    print(f"  🚨 RESPONSE WITH DATA: {res_ov.stdout[:100]}", flush=True)
else:
    print(f"  Response: {res_ov.stdout[:100]}", flush=True)
